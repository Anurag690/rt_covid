        ...
        return(
            <div key={index}>
                ...
                >
                    ...
                    {payload && payload.map((item, index)=><div key={index} className="contents" style={{display: 'flex', flexDirection: 'column', fontSize: '12px'}}>{item.name!="smoothed_9d"&&`${item.name}: ${((item.value+"").split(",").map((x,y)=>x && (+x).toFixed(myToFixed))).join(" ~ ")}`}</div>)}
                </div>}
            </div>
        );
    }
